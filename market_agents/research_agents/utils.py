...
def load_prompts(prompt_path: Path = Path("./market_agents/research_agents/web_search_prompt.yaml")) -> dict:
...
def load_config(config_path: str = "market_agents/research_agents/web_search_config.yaml", 
                prompt_path: str = "market_agents/research_agents/web_search_prompt.yaml") -> tuple[Any, Dict]:
...
def parse_ai_response(response: LLMOutput, has_data: bool = False) -> Dict[str, Any]:
    """Parse AI response with error handling and JSON cleaning."""
    try:
        if not response:
            logger.error("Response object is None")
            return get_default_summary(has_data)
        
        # First try to get the parsed JSON object directly
        if response.json_object and hasattr(response.json_object, 'object'):
            return response.json_object.object
            
        # Then try string content if available
        if response.str_content:
            ...
            try:
                ...
                logger.warning("JSON decode error, falling back to text response structuring")
                return structure_text_response(content, has_data)
